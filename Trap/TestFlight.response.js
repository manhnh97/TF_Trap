...
class i {
  ...
  static getItem(i = new String(), a = null) {
    let t = a;
    if (!0 === i.startsWith("@")) {
      const { key: a, path: m } = i.match(this.#e)?.groups;
      i = a;
      let s = this.getItem(i, {});
      "object" != typeof s && (s = {}), (t = e.get(s, m));
      try {
        t = JSON.parse(t);
      } catch (e) {}
    } else {
  ...
  static setItem(i = new String(), a = new String()) {
    ...
    if (!0 === i.startsWith("@")) {
      const { key: m, path: s } = i.match(this.#e)?.groups;
      i = m;
  ...
  static sendMessageTelegram(a = new String()) {
  ...
  static removeItem(i) {
    let a = !1;
    if (!0 === i.startsWith("@")) {
      const { key: t, path: m } = i.match(this.#e)?.groups;
      i = t;
      let s = this.getItem(i);
      "object" != typeof s && (s = {}),
        (keyValue = e.unset(s, m)),
        (a = this.setItem(i, s));
    } else
      switch (this.#i()) {
        case "Surge":
  ...
  static #t(e = this.dataFile) {
    if (this.isNode()) {
      ...
      t
        ? this.fs.writeFileSync(i, s)
        : m
        ? this.fs.writeFileSync(a, s)
        : this.fs.writeFileSync(i, s);
    }
  }
}
class a {
  ...
  async runScript(e, a) {
    ...
    t = t?.replace?.(/\n/g, "")?.trim();
    let m = i.getItem("@chavy_boxjs_userCfgs.httpapi_timeout");
    (m = 1 * m ?? 20), (m = a?.timeout ?? m);
    const [s, n] = t.split("@"),
      l = {
        url: `http://${n}/v1/scripting/evaluate`,
        body: { script_text: e, mock_type: "cron", timeout: m },
      ...
      default:
        return (
          ...
          await new Promise((e, a) => {
            $httpClient[t](i, (t, m, s) => {
              t
                ? a(t)
                : ((m.ok = /^2\d\d$/.test(m.status)),
                  (m.statusCode = m.status),
                  s &&
                    ((m.body = s), 1 == i["binary-mode"] && (m.bodyBytes = s)),
                  e(m));
            });
          })
        );
...
function S(a, t, m) {
  ...
  let {
    ...
    const l = {
      Settings: m?.Default?.Settings || {},
      Configs: m?.Default?.Configs || {},
      Caches: {},
    };
    Array.isArray(t) || (t = [t]);
    for (let e of t)
      (l.Settings = {
        ...
        (l.Configs = { ...l.Configs, ...m?.[e]?.Configs }),
        s?.[e]?.Caches &&
          "string" == typeof s?.[e]?.Caches &&
          (s[e].Caches = JSON.parse(s?.[e]?.Caches)),
        (l.Caches = { ...l.Caches, ...s?.[e]?.Caches });
    return (
      (function e(i, a) {
        for (var t in i) {
          var m = i[t];
          i[t] = "object" == typeof m && null !== m ? e(m, a) : a(t, m);
        }
        return i;
      })(
        l.Settings,
  ...
  if (
    (s?.Tabs &&
      ...
      e.set(s, "Tabs", s?.Tabs ? [s.Tabs.toString()] : []),
    s?.Domains &&
      !Array.isArray(s?.Domains) &&
      e.set(s, "Domains", s?.Domains ? [s.Domains.toString()] : []),
    s?.Functions &&
      !Array.isArray(s?.Functions) &&
      e.set(s, "Functions", s?.Functions ? [s.Functions.toString()] : []),
    console.log(
      `✅ Set Environment Variables, Settings: ${typeof s}, Settings内容: ${JSON.stringify(
        s
      )}`,
      ""
    ),
  ...
  return { Settings: s, Caches: n, Configs: l };
}
const b = new a(" iRingo: ✈ TestFlight v3.2.0(1) response"),
  U = new (class {
    ...
    #m(e, i = void 0) {
      ...
      if (
        ...
        (e.username || i?.username) &&
          (this.username = e.username || i.username),
        (e.password || i?.password) &&
          (this.password = e.password || i.password),
        (e.host || i?.host) &&
...
b.log(`⚠ url: ${U.toJSON()}`, "");
const C = $request.method,
  L = U.hostname,
  k = U.pathname,
  R = U.paths;
b.log(`⚠ METHOD: ${C}, HOST: ${L}, PATH: ${k}`, "");
const M = (
  $response.headers?.["Content-Type"] ?? $response.headers?.["content-type"]
)?.split(";")?.[0];
function P(e) {
  switch (e.platform || e.name) {
    case "ios":
      b.log("🚧 ios", ""), (e = i(e));
      break;
    case "osx":
      b.log("🚧 osx", ""),
        !0 === e?.macBuildCompatibility?.runsOnAppleSilicon &&
          (b.log("🚧 runsOnAppleSilicon", ""), (e = i(e)));
      break;
    case "appletvos":
      b.log("🚧 appletvos", "");
      break;
    default:
      b.log(`🚧 unknown platform: ${e.platform || e.name}`, "");
  }
  return e;
  function i(e) {
    return (
      (e.compatible = !0),
      (e.platformCompatible = !0),
      (e.hardwareCompatible = !0),
      (e.osCompatible = !0),
      e?.permission && (e.permission = "install"),
      e?.deviceFamilyInfo &&
        (e.deviceFamilyInfo = [
          {
            number: 1,
            name: "iOS",
            iconUrl:
              "https://itunesconnect-mr.itunes.apple.com/itc/img/device-icons/device_family_icon_1.png",
          },
          {
            number: 2,
            name: "iPad",
            iconUrl:
              "https://itunesconnect-mr.itunes.apple.com/itc/img/device-icons/device_family_icon_2.png",
          },
          {
            number: 3,
            name: "Apple TV",
            iconUrl:
              "https://itunesconnect-mr.itunes.apple.com/itc/img/device-icons/device_family_icon_3.png",
          },
        ]),
      e?.compatibilityData?.compatibleDeviceFamilies &&
        (e.compatibilityData.compatibleDeviceFamilies = [
          {
            name: "iPad",
          ...
          { name: "Mac", minimumSupportedDevice: null, unsupportedDevices: [] },
        ]),
      e.macBuildCompatibility &&
        ((e.macBuildCompatibility.runsOnIntel = !0),
        (e.macBuildCompatibility.runsOnAppleSilicon = !0)),
      e
    );
  }
}
              ...
              switch (k) {
                case "/v1/session/authenticate":
                  if (!0 === e.MultiAccount) {
                    ...
                    a?.data
                      ? (b.log("⚠ 有Caches.data", ""),
                        t?.data?.accountId === a?.data?.accountId &&
                          (b.log("⚠ Account ID相等，刷新缓存", ""),
                          (a.headers = {
                            "X-Request-Id": e,
                            "X-Session-Id": m,
                            "X-Session-Digest": s,
                                  ...
                                  if (!0 === e.Universal)
                                    b.log("🚧 启用通用应用支持", ""),
                                      null === t.error &&
                                        ...
                                        (t.data = t.data.map(
                                          (e) => (
                                            !1 !== e.previouslyTested &&
                                              (b.log("🚧 不是前测试人员", ""),
                                              (e.platforms = e.platforms.map(
                                                (e) => (
                                                  (e.build = P(e.build)), e
                                                )
                                              ))),
